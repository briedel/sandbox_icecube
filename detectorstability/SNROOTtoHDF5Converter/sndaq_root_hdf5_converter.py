...
# The variables are named according to this
...
class sncandtabledesc(tables.IsDescription):
    ...
    """
    gps_time = tables.UInt64Col() # Time in ns, left bin edge
    base_time = tables.UInt32Col() # Binning in ms, 500 ms is base bin bize
    NMAvBins = tables.Int32Col() # Size of ring buffer
    NRebins = tables.Int32Col() # Binning multipler wrt 500 ms
    Signal = tables.Float64Col() # Deviation of detector noise rate from mean
    SignalError = tables.Float64Col() # Error on the signal
    Chi2 = tables.Float64Col() # Measure of uniformity of illumination
    ActiveChannels = tables.Int32Col() # No. of active chanels
    RebinIndex = tables.Int32Col() # Position within ring buffer

    #################################################
    # Stuff that doesn't work at the moment
    # NNewActOMs = tables.Int32Col() # Number of DOMs back in analysis
    # NewActOMs = tables.Int32Col(shape = (5160,)) # DOMs added back
    # NNewInactOMs = tables.Int32Col() # Number of DOMs removed from analysis
    # NewInactOMs = tables.Int32Col(shape = (5160,)) # DOMs removed

    NCandidate = tables.Int32Col() # Candiate number
    NChannels = tables.Int32Col() # Number of channels in geometry
    NEvent = tables.Int32Col() # Event number
    # The detector has 5160 DOMs. The maximum size of the data array is therefore 5160
    ChannelRate = tables.Float64Col(shape = (5160,)) # Rate of every channel in analysis bin
    ChannelMean = tables.Float64Col(shape = (5160,)) # Mean rate of every channel in background
    ChannelSigmaSquare = tables.Float64Col(shape = (5160,)) # Variance of rate of every channel in background
    ChannelWeighedDev = tables.Float64Col(shape = (5160,)) # Channel weight (only QE at the moment)

class snrouttabledesc(tables.IsDescription):
    """
    Class to describe the format of the SNDAQ analysis 
    class output table
    For description of variables see sncandtabledesc
    """
...
class snalltabledesc(tables.IsDescription):
    ...
    gps_time = tables.UInt64Col() # Time in ns, left bin edge
    # The detector has 5160 DOMs. The maximum size of the data array is therefore 5160
    counts = tables.Int32Col(shape = (5160,)) # Scaler count for every channel
...
def get_daq_time_as_long(gps_time_high, gps_time_low):
    """
...
def correct_missing_doms(scalerarray, no_channels):
    """ 
...
def set_branches(snobject, snmemory, names):
    """
...
def extract_sn_rout(snrout, snroutrow):
    """
    Extract snrout tree (analysis output) from file
...
def extract_sn_cand(sncand, sncandrow):
    """
    Extract sncand (possible supernova candidates) tree from file
    ...
    :param sncandrow: Pointer to HDF5 row object that referes to a row in the table 
                      we are writing to
    """
    ...
    # Setting pointers in to right memory locations
    # There is some data redundacy with the analysis output for
    # the same time index. 
...
# def get_sn_om_watch(snomwatch, snomwatchrow):
#     """
#     IGNORE
...
def get_base_outfilename_run_number(fnroot):
    """
...
def convert_omkey_domnum(omkey):
    """
...
def get_geo_mapping(fn, fullfile = False):
    """
...
def check_file_exist_open(filename):
    """
...
def get_geo_mapping_filename(runnum, filenum, mapdir = None, mapname = None):
    """
...
def create_out_file(outfn):
    """
    Creating the output HDF5 file
    
    :param outfn: Output file name
    :returns: Pointers to HDF5 file, and tables, and rows in the HDF5 files
    """
    h5file = tables.open_file(outfn, "w")
    
    # Setting highest possible compression level
    filtercomp = tables.Filters(complevel = 9) 
    
    # Creating the necessary tables
    sncandtable = h5file.createTable("/", 'sncand', sncandtabledesc, "SN candidates", filters = filtercomp)
    sncandrow = sncandtable.row

    snrouttable = h5file.createTable("/", 'snrout', snrouttabledesc, "SN Analysis Class Results", filters = filtercomp)
    snroutrow = snrouttable.row

    snalltable = h5file.createTable("/", 'snall', snalltabledesc, "500 ms of DOM ", filters = filtercomp)
    snallrow = snalltable.row

    # snomwatchtable = h5file.createTable("/", 'snomwatch', snomwatchtabledesc, "Monitoring quantities for channels ")
    # snomwatchrow = snomwatchtable.row
    
    return h5file, sncandtable, sncandrow, snrouttable, snroutrow, snalltable, snallrow
...
def main(options, args):
    ...
    :param options: Options to script
    :param args: List of input files
    """
    # Checking if we are in Condor world and we have a local scratch disk we can
    # untar things into
    if "_CONDOR_SCRATCH_DIR" in os.environ.keys():
        fnroot = untar_files(args, "%s/" % os.environ["_CONDOR_SCRATCH_DIR"])
    elif options.outdir:
        fnroot = untar_files(args, options.outdir)
    else:
        fnroot = untar_files(args, os.path.dirname(options.outfile))
    
    if options.outfile:
        h5file, sncandtable, sncandrow, snrouttable, snroutrow, snalltable, snallrow = create_out_file(options.outfile)
    previous_run_num = 0
    ...
    for fr in fnroot:
        ...
        if not options.outfile and previous_run_num != runNum:
            if previous_run_num != 0:
                ...
                h5file.close()
            h5file, sncandtable, sncandrow, snrouttable, snroutrow, snalltable, snallrow = create_out_file(outfn)
